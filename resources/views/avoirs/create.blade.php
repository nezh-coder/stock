        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Créer un Avoir - Retour d'articles</h3>
                    ...
                    <form action="{{ route('avoirs.store') }}" method="POST" id="avoirForm">
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="client_id" id="client_id" class="form-control @error('client_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($clients as $client)
                                            <option value="{{ $client->id }}" {{ (isset($source) && $source->client_id == $client->id) || old('client_id') == $client->id ? 'selected' : '' }}>
                                                {{ $client->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date_avoir" id="date_avoir" class="form-control @error('date_avoir') is-invalid @enderror"
                                           value="{{ old('date_avoir', date('Y-m-d')) }}" required>
                                    @error('date_avoir')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                         ...
                         <div class="form-group mt-3">
                            ...
                            <textarea name="motif" id="motif" class="form-control @error('motif') is-invalid @enderror" rows="2">{{ old('motif') }}</textarea>
                            @error('motif')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="bon_livraison_id" id="bon_livraison_id" class="form-control @error('bon_livraison_id') is-invalid @enderror">
                                        ...
                                        @foreach($bonLivraisons as $bonLivraison)
                                            <option value="{{ $bonLivraison->id }}" data-type="bon_livraison" {{ (isset($source) && $sourceType === 'bon_livraison' && $source->id == $bonLivraison->id) || old('bon_livraison_id') == $bonLivraison->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="facture_id" id="facture_id" class="form-control @error('facture_id') is-invalid @enderror">
                                        ...
                                        @foreach($factures as $facture)
                                            <option value="{{ $facture->id }}" data-type="facture" {{ (isset($source) && $sourceType === 'facture' && $source->id == $facture->id) || old('facture_id') == $facture->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group mt-4">
                            <button type="submit" class="btn btn-primary btn-lg">
                                <i class="fas fa-save"></i> Créer l'Avoir
                            </button>
...
$(document).ready(function() {
    ...
    // Load products from source if available on page load
    @if(isset($source) && $source && isset($sourceType))
        @if($sourceType === 'bon_livraison')
            loadDocumentProducts('{{ $source->id }}', 'bon_livraison');
        @elseif($sourceType === 'facture')
            loadDocumentProducts('{{ $source->id }}', 'facture');
        @endif
    @endif
    ...
    // Load products from selected document (Bon de Livraison or Facture)
